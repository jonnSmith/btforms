...
class GFCommon{
    ...
    public static function check_update($option, $cache=false){
    ...
    public static function get_checkbox_choices($field, $value, $disabled_text){
        ...
        if(is_array($field["choices"])){
            ...
            foreach($field["choices"] as $choice){
                ...
                $choices.= sprintf("<div class='checkbox'><input name='input_%s' type='checkbox' $logic_event value='%s' %s id='choice_%s' $tabindex %s /><label for='choice_%s'>%s</label></div>", $input_id, esc_attr($choice_value), $checked, $id, $disabled_text, $id, $choice["text"]);
    ...
    public static function get_radio_choices($field, $value="", $disabled_text){
        ...
        if(is_array($field["choices"])){
            ...
            foreach($field["choices"] as $choice){
                ...
                $choices .= sprintf("<div class='radio'><input name='input_%d' type='radio' value='%s' %s id='choice_%s' $tabindex %s $logic_event %s />%s</div>", $field["id"], esc_attr($field_value), $checked, $id, $disabled_text, $input_focus, $label);
    ...
    private static function get_number_dropdown($name, $id, $selected_value, $tabindex, $disabled_text, $placeholder, $start_number, $end_number){
        $str = "<select class='form-control' name='{$name}' id='{$id}' {$tabindex} {$disabled_text} >";
    ...
    public static function get_field_input($field, $value="", $lead_id=0, $form_id=0){
        ...
        $bootstrap_container_classes = "form-group ";
        $help_block = '';

        if($field['isRequired'] == '1') {
            $required_attributes .= "required";
            $bootstrap_classes .= "required ";
            $help_block .= '<span class="help-block with-errors"></span>';
        }
        ...
        switch(RGFormsModel::get_input_type($field)){
            ...
            case "singleproduct" :
                ...
                if(IS_ADMIN){
                    ...
                    $tabindex = self::get_tabindex();
                    $quantity_field .= " <label>" . apply_filters("gform_product_quantity_{$form_id}",apply_filters("gform_product_quantity",__("Quantity:", "gravityforms"), $form_id), $form_id) . "</label> <input type='number' name='input_{$id}.3' value='{$quantity}' id='ginput_quantity_{$form_id}_{$field["id"]}' $required_attributes class='$bootstrap_classes' size='10' {$tabindex}/>";
                }
                else{
                ...
                return "<div class='row'>
                        ...
                        <div class='$bootstrap_container_classes col-md-12'>
                        <input type='hidden' name='input_{$id}.1' value='{$product_name}' class='gform_hidden' />
                        <label>" . apply_filters("gform_product_price_{$form_id}", apply_filters("gform_product_price",__("Price", "gravityforms"), $form_id), $form_id) . ":</label>
                        <span class='ginput_product_price' id='{$field_id}'>" . esc_html(GFCommon::to_money($price, $currency)) ."</span>
                        </div>

                        <div class='$bootstrap_container_classes col-md-12'>
                        <input type='hidden' name='input_{$id}.2' id='ginput_base_price_{$form_id}_{$field["id"]}' class='gform_hidden' value='" . esc_attr($price) . "'/>{$quantity_field}
                        $help_block
                        </div>
            ...
            case "email":
                ...
                $html_input_type = RGFormsModel::is_html5_enabled() ? "email" : "email";
                ...
                if(IS_ADMIN && RG_CURRENT_VIEW != "entry"){
                    ...
                    if(rgget("emailConfirmEnabled", $field) && RG_CURRENT_VIEW != "entry"){
                        ...
                        return "<div class='".$bootstrap_container_classes."' id='{$field_id}_container'><span id='{$field_id}_1_container'><label for='{$field_id}'>" . apply_filters("gform_email_{$form_id}", apply_filters("gform_email",__("Enter Email", "gravityforms"), $form_id), $form_id) . "</label><input type='{$html_input_type}' name='input_{$id}' id='{$field_id}' class='".$bootstrap_classes."' ".$required_attributes." value='" . esc_attr($value) . "' {$first_tabindex} {$logic_event} {$disabled_text}/></span><span id='{$field_id}_2_container' class='ginput_right'><label for='{$field_id}_2'>" . apply_filters("gform_email_confirm_{$form_id}", apply_filters("gform_email_confirm",__("Confirm Email", "gravityforms"), $form_id), $form_id) . "</label><input type='{$html_input_type}' name='input_{$id}_2' id='{$field_id}_2' class='".$bootstrap_classes."' value='" . esc_attr(rgpost("input_" . $id ."_2")) . "' {$last_tabindex} {$disabled_text}/></span>".$help_block."<div class='gf_clear gf_clear_complex'></div></div>";
                    }
                    else{
                        $tabindex = self::get_tabindex();
                        return sprintf("<div class='".$bootstrap_container_classes."'><input name='input_%d' id='%s' type='%s' value='%s' placeholder='".$field['defaultValue']."' ".$required_attributes." class='".$bootstrap_classes." %s' {$max_length} {$tabindex} {$html5_attributes} {$logic_event} %s/>".$help_block."</div>", $id, $field_id, $html_input_type, esc_attr($value), esc_attr($class), $disabled_text);
                    }
                }
            ...
            case "number" :
                ...
                $tabindex = self::get_tabindex();
                return sprintf("<div class='form-group'><input name='input_%d' id='%s' type='number' {$step_attr} value='%s' class='".$bootstrap_classes." %s' {$tabindex} {$logic_event} {$read_only} %s/>%s $help_block</div>", $id, $field_id, esc_attr($value), esc_attr($class),  $disabled_text, $instruction);
            ...
            case "phone" :
                ...
                $tabindex = self::get_tabindex();
                if(IS_ADMIN && RG_CURRENT_VIEW != "entry") {
                    return sprintf("<div class='ginput_container'><input name='input_%d' id='%s' type='{$html_input_type}' placeholder='%s' class='%s' {$tabindex} {$logic_event} %s/>{$instruction_div}</div>", $id, $field_id, esc_attr($value), esc_attr($class), $disabled_text);
                } else {
                    return sprintf("<div class='" . $bootstrap_container_classes . "'><input placeholder='".$field['defaultValue']."' name='input_%d'  id='%s' type='{$html_input_type}' value='%s' $required_attributes class='$bootstrap_classes $class' {$tabindex} {$logic_event} %s/>{$instruction_div}$help_block</div>", $id, $field_id, esc_attr($value), esc_attr($class), $disabled_text);
                }
            ...
            case "textarea":
                ...
                if(IS_ADMIN && RG_CURRENT_VIEW != "entry") {
                    return sprintf("<div class='ginput_container'><textarea name='input_%d' id='%s' class='form-control %s' {$tabindex} {$logic_event} %s rows='10' cols='50' value='%s'></textarea></div>{$max_chars}", $id, $field_id, esc_attr($class), $disabled_text, esc_html($value));
                } else {
                    return sprintf("<div class='" . $bootstrap_container_classes . "'><textarea placeholder='".$field['defaultValue']."' name='input_%d' id='%s' $required_attributes class='$bootstrap_classes %s' {$tabindex} {$logic_event} %s rows='10' cols='50' value='%s'></textarea>$help_block</div>{$max_chars}", $id, $field_id, esc_attr($class), $disabled_text, esc_html($value));
                }
            ...
            case "post_custom_field":
                ...
                return !empty($post_link) ? $post_link : sprintf("<div class='" . $bootstrap_container_classes . "'><input name='input_%d' id='%s' type='text' value='%s' $required_attributes class='$bootstrap_classes' {$tabindex} {$logic_event} %s/>$help_block</div>", $id, $field_id, esc_attr($value), esc_attr($class), $disabled_text);
            break;
            ...
            case "post_excerpt":
                ...
                $tabindex = self::get_tabindex();
                return !empty($post_link) ? $post_link : sprintf("<div class='" . $bootstrap_container_classes . "'><textarea name='input_%d' id='%s' $required_attributes class='$bootstrap_classes' {$tabindex} {$logic_event} %s rows='10' cols='50'>%s</textarea>$help_block</div>{$max_chars}", $id, $field_id, esc_attr($class), $disabled_text, esc_html($value));
            break;
            ...
            case "post_category" :
                ...
                if(rgget("displayAllCategories", $field) && !IS_ADMIN){
                    ...
                    return "<div class='ginput_container'>" . wp_dropdown_categories($args) . "$help_block</div>";
                }
                else{
                    $tabindex = self::get_tabindex();
                    if(is_array(rgar($field, "choices")))
            ...
            case "post_image" :
                ...
                $tabindex = self::get_tabindex();
                $upload = sprintf("<div class='$bootstrap_container_classes$class_suffix'>{$preview}$file_label<input name='input_%d' id='%s' type='file' value='%s' class='%s' $tabindex %s/></div>", $id, $field_id, esc_attr($value), esc_attr($class . $hidden_class), $disabled_text);

                $tabindex = self::get_tabindex();
                $title_field = rgget("displayTitle", $field) || IS_ADMIN ? sprintf("<div class='$bootstrap_container_classes$class_suffix ginput_post_image_title' $title_style><label for='%s_1'>" . apply_filters("gform_postimage_title_{$form_id}",apply_filters("gform_postimage_title",__("Title", "gravityforms"), $form_id), $form_id) . "</label><input type='text' name='input_%d.1' id='%s_1' value='%s' class='$bootstrap_classes' $tabindex %s/>$help_block</div>", $id, $field_id, $title, $disabled_text, $field_id) : "";

                $tabindex = self::get_tabindex();
                $caption_field = rgget("displayCaption", $field) || IS_ADMIN ? sprintf("<div class='$bootstrap_container_classes$class_suffix ginput_post_image_caption' $caption_style><label for='%s_4'>" . apply_filters("gform_postimage_caption_{$form_id}",apply_filters("gform_postimage_caption",__("Caption", "gravityforms"), $form_id), $form_id) . "</label><input type='text' name='input_%d.4' id='%s_4' value='%s' class='$bootstrap_classes' $tabindex %s/>$help_block</div>", $id, $field_id, $caption, $disabled_text, $field_id) : "";

                $tabindex = self::get_tabindex();
                $description_field = rgget("displayDescription", $field) || IS_ADMIN? sprintf("<div class='$bootstrap_container_classes$class_suffix ginput_post_image_description' $description_style><label for='%s_7'>" . apply_filters("gform_postimage_description_{$form_id}",apply_filters("gform_postimage_description",__("Description", "gravityforms"), $form_id), $form_id) . "</label><input type='text' name='input_%d.7' id='%s_7' value='%s' class='$bootstrap_classes' $tabindex %s/>$help_block</div>", $id, $field_id, $description, $disabled_text, $field_id) : "";

                return "<div class='" . $bootstrap_container_classes . "'>" . $upload . $title_field . $caption_field . $description_field . "</div>";
            ...
            case "multiselect" :
                ...
                return sprintf("<div class='$bootstrap_container_classes'><select multiple='multiple' {$placeholder} size='{$size}' name='input_%d[]' id='%s' {$logic_event} class='$bootstrap_classes bootstrap-select %s' $tabindex %s>%s</select></div>", $id, $field_id, $css_class, $disabled_text, self::get_select_choices($field, $value));
            ...
            case "select" :
                ...
                $css_class = trim(esc_attr($class) . " gfield_select");
                $tabindex = self::get_tabindex();
                return sprintf("<div class='".$bootstrap_container_classes."'><select name='input_%d' id='%s' $logic_event class='".$bootstrap_classes." bootstrap-select %s' $tabindex %s>%s</select>$help_block</div>", $id, $field_id, $css_class, $disabled_text, self::get_select_choices($field, $value));
            ...
            case "checkbox" :
                ...
                if(IS_ADMIN && RG_CURRENT_VIEW != "entry") {
                    ...
                    return sprintf("<div class='form-group' id='%s'>%s</div>", $field_id, self::get_checkbox_choices($field, $value, $disabled_text));
                }
            ...
            case "radio" :
                ...
                if(IS_ADMIN && RG_CURRENT_VIEW != "entry") {
                    ...
                    return sprintf("<div class='$bootstrap_container_classes' id='%s'>%s</div>", $field_id, self::get_radio_choices($field, $value, $disabled_text));
                }
            ...
            case "name" :
                ...
                if(IS_ADMIN && RG_CURRENT_VIEW != "entry") {
                    ...
                    switch(rgget("nameFormat", $field)){
                        ...
                        case "simple" :
                            $tabindex = self::get_tabindex();
                            return sprintf("<div class='ginput_container'><input name='input_%d' id='%s' type='text' placeholder='%s' class='%s' $tabindex %s/></div>", $id, $field_id, esc_attr($value), esc_attr($class), $disabled_text);
                    ...
                    switch (rgget("nameFormat", $field)) {
                        ...
                        case "extended" :
                            ...
                            return sprintf("<div class='row' id='$field_id'>
                        <div id='" . $field_id . "_2_container' class='name_prefix " . $bootstrap_container_classes . " col-md-3' >
                        <input type='text' name='input_%d.2' id='%s_2' value='%s' $required_attributes class='$bootstrap_classes' $prefix_tabindex %s/>
                        <label for='%s_2'>" . apply_filters("gform_name_prefix_{$form_id}", apply_filters("gform_name_prefix", __("Prefix", "gravityforms"), $form_id), $form_id) . "</label>
                        $help_block
                        </div>
                        <div id='" . $field_id . "_3_container' class='name_first " . $bootstrap_container_classes . " col-md-3'>
                        <input type='text' name='input_%d.3' id='%s_3' value='%s' $required_attributes class='$bootstrap_classes' $first_tabindex %s/>
                        <label for='%s_3'>" . apply_filters("gform_name_first_{$form_id}", apply_filters("gform_name_first", __("First", "gravityforms"), $form_id), $form_id) . "</label>
                        $help_block
                        </div>
                        <div id='" . $field_id . "_6_container' class='name_last " . $bootstrap_container_classes . " col-md-3'>
                        <input type='text' name='input_%d.6' id='%s_6' value='%s' $required_attributes class='$bootstrap_classes' $last_tabindex %s/>
                        <label for='%s_6'>" . apply_filters("gform_name_last_{$form_id}", apply_filters("gform_name_last", __("Last", "gravityforms"), $form_id), $form_id) . "</label>
                        $help_block
                        </div>
                        <div id='" . $field_id . "_8_container' class='name_suffix " . $bootstrap_container_classes . " col-md-3'>
                        <input type='text' name='input_%d.8' id='%s_8' value='%s' $required_attributes class='$bootstrap_classes' $suffix_tabindex %s/>
                        <label for='%s_8'>" . apply_filters("gform_name_suffix_{$form_id}", apply_filters("gform_name_suffix", __("Suffix", "gravityforms"), $form_id), $form_id) . "</label>
                        $help_block
                        </div>
                        </div>", $id, $field_id, $prefix, $disabled_text, $field_id, $id, $field_id, $first, $disabled_text, $field_id, $id, $field_id, $last, $disabled_text, $field_id, $id, $field_id, $suffix, $disabled_text, $field_id);

                        case
                        "simple" :
                            $tabindex = self::get_tabindex();
                            return sprintf("<div class='" . $bootstrap_container_classes . "'><input name='input_%d' id='%s' type='text' placeholder='".$field['defaultValue']."' value='%s' $required_attributes class='$bootstrap_classes %s' $tabindex %s/>$help_block</div>", $id, $field_id, esc_attr($value), esc_attr($class), $disabled_text);

                        default :
                            $first_tabindex = self::get_tabindex();
                            $last_tabindex = self::get_tabindex();
                            return sprintf("<div class='row' id='$field_id'>
                        <div id='" . $field_id . "_3_container' class='" . $bootstrap_container_classes . " col-md-6'>
                        <label for='%s_3'>" . apply_filters("gform_name_first_{$form_id}", apply_filters("gform_name_first", __("First", "gravityforms"), $form_id), $form_id) . "</label>
                        <input type='text' name='input_%d.3' id='%s_3' $required_attributes class='$bootstrap_classes' value='%s' $first_tabindex %s/>
                        $help_block
                        </div>
                        <div id='" . $field_id . "_6_container' class='" . $bootstrap_container_classes . " col-md-6'>
                        <label for='%s_6'>" . apply_filters("gform_name_last_{$form_id}", apply_filters("gform_name_last", __("Last", "gravityforms"), $form_id), $form_id) . "</label>
                        <input type='text' name='input_%d.6' id='%s_6' $required_attributes class='$bootstrap_classes' value='%s' $last_tabindex %s/>
                        $help_block
                        </div>
                        <div class='gf_clear gf_clear_complex'></div>
                        </div>", $id, $field_id, $first, $disabled_text, $field_id, $id, $field_id, $last, $disabled_text, $field_id);
            ...
            case "address" :
                ...
                if(IS_ADMIN && RG_CURRENT_VIEW != "entry") {
                    ...
                    $tabindex = self::get_tabindex();
                    $street_address = sprintf("<div class='ginput_full$class_suffix $bootstrap_container_classes col-md-12' id='" . $field_id . "_1_container'>
                    <label for='%s_1' id='" . $field_id . "_1_label'>" . apply_filters("gform_address_street_{$form_id}", apply_filters("gform_address_street", __("Street Address", "gravityforms"), $form_id), $form_id) . "</label>
                    <input type='text' $required_attributes class='$bootstrap_classes' name='input_%d.1' id='%s_1' value='%s' $tabindex %s/>
                    $help_block
                    </div>
                    ", $id, $field_id, $street_value, $disabled_text, $field_id);
                    ...
                    if (IS_ADMIN || !rgget("hideAddress2", $field)) {
                        $tabindex = self::get_tabindex();
                        $street_address2 = sprintf("<div class='ginput_full$class_suffix $bootstrap_container_classes col-md-12' id='" . $field_id . "_2_container' $style>
                        <label for='%s_2' id='" . $field_id . "_2_label'>" . apply_filters("gform_address_street2_{$form_id}", apply_filters("gform_address_street2", __("Address Line 2", "gravityforms"), $form_id), $form_id) . "</label>
                        <input type='text' name='input_%d.2' id='%s_2' $required_attributes class='$bootstrap_classes' value='%s' $tabindex %s/>
                        $help_block
                        </div>", $id, $field_id, $street2_value, $disabled_text, $field_id);
                    }
                    ...
                    if ($address_display_format == "zip_before_city") {
                        ...
                        $tabindex = self::get_tabindex();
                        $zip = sprintf("
                        <div class='ginput_{$zip_location}$class_suffix $bootstrap_container_classes col-md-12' id='" . $field_id . "_5_container'>
                        <input type='text' name='input_%d.5' id='%s_5' value='%s' $required_attributes class='$bootstrap_classes' $tabindex %s/>
                        <label for='%s_5' id='" . $field_id . "_5_label'>" . apply_filters("gform_address_zip_{$form_id}", apply_filters("gform_address_zip", $zip_label, $form_id), $form_id) . "</label>
                        </div>", $id, $field_id, $zip_value, $disabled_text, $field_id);

                        //city field
                        $tabindex = self::get_tabindex();
                        $city = sprintf("<div class='ginput_{$city_location}$class_suffix $bootstrap_container_classes col-md-12' id='" . $field_id . "_3_container'>
                        <input type='text' $required_attributes class='$bootstrap_classes' name='input_%d.3' id='%s_3' value='%s' $tabindex %s/>
                        <label for='%s_3' id='{$field_id}_3_label'>" . apply_filters("gform_address_city_{$form_id}", apply_filters("gform_address_city", __("City", "gravityforms"), $form_id), $form_id) . "</label>
                        </div>", $id, $field_id, $city_value, $disabled_text, $field_id);

                        //state field
                        $style = (IS_ADMIN && rgget("hideState", $field)) ? "style='display:none;'" : "";
                        if (IS_ADMIN || !rgget("hideState", $field)) {
                            $state_field = self::get_state_field($field, $id, $field_id, $state_value, $disabled_text, $form_id);
                            $state = sprintf("<div class='ginput_{$state_location}$class_suffix $bootstrap_container_classes col-md-12' id='" . $field_id . "_4_container' $style>
                            $state_field
                            <label for='%s_4' id='" . $field_id . "_4_label'>" . apply_filters("gform_address_state_{$form_id}", apply_filters("gform_address_state", $state_label, $form_id), $form_id) . "</label>
                            </div>", $field_id);
                        } else {
                            $state = sprintf("<input type='hidden' class='gform_hidden' name='input_%d.4' id='%s_4' value='%s'/>", $id, $field_id, $state_value);
                        }
                        ...
                        //city field
                        $tabindex = self::get_tabindex();
                        $city = sprintf("<div class='ginput_{$city_location}$class_suffix $bootstrap_container_classes col-md-4' id='" . $field_id . "_3_container'>
                        <label for='%s_3' id='$field_id.3_label'>" . apply_filters("gform_address_city_{$form_id}", apply_filters("gform_address_city", __("City", "gravityforms"), $form_id), $form_id) . "</label>
                        <input type='text' $required_attributes class='$bootstrap_classes' name='input_%d.3' id='%s_3' value='%s' $tabindex %s/>
                        $help_block
                        </div>", $id, $field_id, $city_value, $disabled_text, $field_id);
                        ...
                        if (IS_ADMIN || !rgget("hideState", $field)) {
                            $state_field = self::get_state_field($field, $id, $field_id, $state_value, $disabled_text, $form_id);
                            $state = sprintf("<div class='ginput_{$state_location}$class_suffix $bootstrap_container_classes col-md-4' id='" . $field_id . "_4_container' $style>
                            <label for='%s_4' id='" . $field_id . "_4_label'>" . apply_filters("gform_address_state_{$form_id}", apply_filters("gform_address_state", $state_label, $form_id), $form_id) . "</label>
                            $state_field
                            $help_block
                            </div>", $field_id);
                        ...
                        //zip field
                        $tabindex = self::get_tabindex();
                        $zip = sprintf("<div class='ginput_{$zip_location}$class_suffix $bootstrap_container_classes col-md-4' id='" . $field_id . "_5_container'>
                        <label for='%s_5' id='" . $field_id . "_5_label'>" . apply_filters("gform_address_zip_{$form_id}", apply_filters("gform_address_zip", $zip_label, $form_id), $form_id) . "</label>
                        <input $required_attributes class='$bootstrap_classes' type='text' name='input_%d.5' id='%s_5' value='%s' $tabindex %s/>
                        $help_block
                        </div>", $id, $field_id, $zip_value, $disabled_text, $field_id);
                    ...
                    if (IS_ADMIN || !$hide_country) {
                        $style = $hide_country ? "style='display:none;'" : "";
                        $tabindex = self::get_tabindex();
                        $country = sprintf("<div class='ginput_{$country_location}$class_suffix $bootstrap_container_classes col-md-12' id='" . $field_id . "_6_container' $style>
                        <label for='%s_6' id='" . $field_id . "_6_label'>" . apply_filters("gform_address_country_{$form_id}", apply_filters("gform_address_country", __("Country", "gravityforms"), $form_id), $form_id) . "</label>
                        <select $required_attributes class='$bootstrap_classes bootstrap-select' name='input_%d.6' id='%s_6' $tabindex %s>%s</select>
                        $help_block
                        </div>", $id, $field_id, $disabled_text, $country_list, $field_id);
                    } else {
                        $country = sprintf("<input type='hidden' class='gform_hidden' name='input_%d.6' id='%s_6' value='%s'/>", $id, $field_id, $country_value);
                    }
                    ...
                    return "<div class='ginput_complex$class_suffix ginput_container row' id='$field_id'>" . $inputs . $help_block.  "<div class='gf_clear gf_clear_complex'></div></div>";
            ...
            case "date" :
                ...
                if(IS_ADMIN && RG_CURRENT_VIEW != "entry"){
                        ...
                        switch($field_position){
                            ...
                            case "dmy" :
                                ...
                                $field_str = $date_type == "datedropdown"
                                            ? "<div class='row'><div class='col-md-4 $bootstrap_container_classes' id='{$field_id}'>" . self::get_day_dropdown("input_{$id}[]", "{$field_id}_1", rgar($date_info,"day"), $tabindex, $disabled_text) . "</div>"
                                            : sprintf("<div class='row'><div class='col-md-4 $bootstrap_container_classes' id='%s'><input type='text' maxlength='2' name='input_%d[]' id='%s_2' value='%s' $tabindex %s/><label for='%s_2'>" . __("DD", "gravityforms") . "</label></div>", $field_id, $id, $field_id, rgget("day", $date_info), $disabled_text, $field_id);

                                $tabindex = self::get_tabindex();
                                $field_str .= $date_type == "datedropdown"
                                            ? "<div class='col-md-4 $bootstrap_container_classes' id='{$field_id}'>" . self::get_month_dropdown("input_{$id}[]", "{$field_id}_1", rgar($date_info,"month"), $tabindex, $disabled_text) . "</div>"
                                            : sprintf("<div class='col-md-4 $bootstrap_container_classes' id='%s'><input type='text' maxlength='2' name='input_%d[]' id='%s_1' value='%s' $tabindex %s/><label for='%s_1'>" . __("MM", "gravityforms") . "</label></div>", $field_id, $id, $field_id, rgget("month", $date_info), $disabled_text, $field_id);

                                $tabindex = self::get_tabindex();
                                $field_str .= $date_type == "datedropdown"
                                        ? "<div class='col-md-4 $bootstrap_container_classes' id='{$field_id}'>" . self::get_year_dropdown("input_{$id}[]", "{$field_id}_1", rgar($date_info,"year"), $tabindex, $disabled_text) . "</div></div>"
                                        : sprintf("<div class='col-md-4 $bootstrap_container_classes' id='%s'><input type='text' maxlength='4' name='input_%d[]' id='%s_3' value='%s' $tabindex %s/><label for='%s_3'>" . __("YYYY", "gravityforms") . "</label></div></div>", $field_id, $id, $field_id, rgget("year", $date_info), $disabled_text, $field_id);
                            ...
                            case "ymd" :
                                $tabindex = self::get_tabindex();
                                $field_str = $date_type == "datedropdown"
                                        ? "<div class='row'><div class='col-md-4 $bootstrap_container_classes' id='{$field_id}'>" . self::get_year_dropdown("input_{$id}[]", "{$field_id}_1", rgar($date_info,"year"), $tabindex, $disabled_text) . "</div>"
                                        : sprintf("<div class='row'><div class='col-md-4 $bootstrap_container_classes' id='%s'><input type='text' class='$bootstrap_classes' maxlength='4' name='input_%d[]' id='%s_3' value='%s' $tabindex %s/><label for='%s_3'>" . __("YYYY", "gravityforms") . "</label></div>", $field_id, $id, $field_id, rgget("year", $date_info), $disabled_text, $field_id);

                                $field_str .= $date_type == "datedropdown"
                                            ? "<div class='col-md-4 $bootstrap_container_classes' id='{$field_id}'>" . self::get_month_dropdown("input_{$id}[]", "{$field_id}_1", rgar($date_info,"month"), $tabindex, $disabled_text) . "</div>"
                                            : sprintf("<div class='col-md-4 $bootstrap_container_classes' id='%s'><input type='text' class='$bootstrap_classes' maxlength='2' name='input_%d[]' id='%s_1' value='%s' $tabindex %s/><label for='%s_1'>" . __("MM", "gravityforms") . "</label></div>", $field_id, $id, $field_id, rgar($date_info,"month"), $disabled_text, $field_id);

                                $tabindex = self::get_tabindex();
                                $field_str .= $date_type == "datedropdown"
                                            ? "<div class='col-md-4 $bootstrap_container_classes' id='{$field_id}'>" . self::get_day_dropdown("input_{$id}[]", "{$field_id}_1", rgar($date_info,"day"), $tabindex, $disabled_text) . "</div></div>"
                                            : sprintf("<div class='col-md-4 form-group' id='%s'><input type='text' class='$bootstrap_classes' maxlength='2' name='input_%d[]' id='%s_2' value='%s' $tabindex %s/><label for='%s_2'>" . __("DD", "gravityforms") . "</label></div></div>", $field_id, $id, $field_id, rgar($date_info,"day"), $disabled_text, $field_id);
                            ...
                            default :
                                ...
                                $field_str = $date_type == "datedropdown"
                                            ? "<div class='row'><div class='col-md-4 $bootstrap_container_classes' id='{$field_id}'>" . self::get_month_dropdown("input_{$id}[]", "{$field_id}_1", rgar($date_info,"month"), $tabindex, $disabled_text) . "</div>"
                                            : sprintf("<div class='row'><div class='col-md-4 $bootstrap_container_classes' id='%s'><input type='text' class='$bootstrap_classes' maxlength='2' name='input_%d[]' id='%s_1' class='form-control' value='%s' $tabindex %s/><label for='%s_1'>" . __("MM", "gravityforms") . "</label></div>", $field_id, $id, $field_id, rgar($date_info,"month"), $disabled_text, $field_id);

                                $tabindex = self::get_tabindex();
                                $field_str .= $date_type == "datedropdown"
                                            ? "<div class='col-md-4 $bootstrap_container_classes' id='{$field_id}'>" . self::get_day_dropdown("input_{$id}[]", "{$field_id}_1", rgar($date_info,"day"), $tabindex, $disabled_text) . "</div>"
                                            : sprintf("<div class='col-md-4 $bootstrap_container_classes' id='%s'><input type='text' class='$bootstrap_classes'  maxlength='2' name='input_%d[]' id='%s_2' class='form-control' value='%s' $tabindex %s/><label for='%s_2'>" . __("DD", "gravityforms") . "</label></div>", $field_id, $id, $field_id, rgar($date_info,"day"), $disabled_text, $field_id);

                                $tabindex = self::get_tabindex();
                                $field_str .= $date_type == "datedropdown"
                                        ? "<div class='col-md-4 $bootstrap_container_classes' id='{$field_id}'>" . self::get_year_dropdown("input_{$id}[]", "{$field_id}_1", rgar($date_info,"year"), $tabindex, $disabled_text) . "</div></div>"
                                        : sprintf("<div class='col-md-4 $bootstrap_container_classes' id='%s'><input type='text' class='$bootstrap_classes' maxlength='4' name='input_%d[]' id='%s_3' class='form-control' value='%s' $tabindex %s/><label for='%s_3'>" . __("YYYY", "gravityforms") . "</label></div></div>", $field_id, $id, $field_id, rgget("year", $date_info), $disabled_text, $field_id);
                    ...
                    else
                    {
                        ...
                        return sprintf("<div class='$bootstrap_container_classes'><input name='input_%d' id='%s' type='text' placeholder='%s' data-date-language='".$locale."' data-date-orientation='top left' data-date-format='".$format_name."' data-date-autoclose='true' data-provide='datepicker' $required_attributes class='$bootstrap_classes datepicker %s %s' $tabindex %s/> </div>", $id, $field_id, esc_attr($value), esc_attr($class), $format, $disabled_text);
                    }
                }
            ...
            case "time" :
                ...
                $ampm_field = is_admin() || rgar($field, "timeFormat") != "24" ? "<div class='col-md-4' {$ampm_field_style}><select class='form-control' name='input_{$id}[]' id='{$field_id}_3' $ampm_tabindex {$disabled_text}><option value='am' {$am_selected}>" . __("AM", "gravityforms") . "</option><option value='pm' {$pm_selected}>" . __("PM", "gravityforms") . "</option></select></div>" : "";
                ...
                if(IS_ADMIN && RG_CURRENT_VIEW != "entry"){
                    ...
                    return sprintf("<div class='row'><div class='$bootstrap_container_classes col-md-4' id='%s'><input type='text' $required_attributes class='$bootstrap_classes' maxlength='2' name='input_%d[]' id='%s_1' value='%s' $hour_tabindex %s/>  <label for='%s_1'>" . "</label>$help_block</div><div class='$bootstrap_container_classes col-md-4'><input type='text' class='$bootstrap_classes' maxlength='2' name='input_%d[]' id='%s_2' value='%s' $minute_tabindex %s/><label for='%s_2'>" . "</label>$help_block</div>{$ampm_field}</div>", $field_id, $id, $field_id, $hour, $disabled_text, $field_id, $id, $field_id, $minute, $disabled_text, $field_id);
                }
            case "fileupload" :
                $tabindex = self::get_tabindex();
            ...
            case "list" :
                ...
                foreach($value as $item){
                    ...
                    foreach($columns as $column){
                        ...
                        $list .= "<td>" . self::get_list_input($field, $has_columns, $column, $val, $form_id) . "</td>";
                        $colnum++;
                    }
                    ...
                    if(rgar($field, "maxRows") != 1){
                        ...
                        $list .="<td>";
                        $list .="   <span class='glyphicon glyphicon-plus' {$disabled_text} title='" . __("Add a row", "gravityforms") . "' alt='" . __("Add a row", "gravityforms") . "' {$on_click} style='cursor:pointer; margin:0 3px;'></span>" .
                                "   <span class='glyphicon glyphicon-minus' title='" . __("Remove this row", "gravityforms") . "' alt='" . __("Remove this row", "gravityforms") . "' style='cursor:pointer;' onclick='gformDeleteListItem(this, {$maxRow})'></span>";
                        $list .="</td>";
    ...
    private static function get_list_input($field, $has_columns, $column, $value, $form_id){
        ...
        switch($input_info["type"]){
            ...
            default :
                $input = "<input type='text' class='form-control' name='input_{$field["id"]}[]' value='" . esc_attr($value) . "' {$tabindex}/>";
            break;
        }
    ...
    private static function get_state_field($field, $id, $field_id, $state_value, $disabled_text, $form_id){
        ...
        if(IS_ADMIN && RG_CURRENT_VIEW != "entry"){
            ...
            $state_field_id = "id='" . $field_id . "_4' class='form-control'";
        }
